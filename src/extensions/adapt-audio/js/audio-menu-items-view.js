define([
  ...
  var AudioMenuItemsView = Backbone.View.extend({
    ...
    render: function () {
      ...
      var template = Handlebars.templates["audioMenuItemsView"];
      if (this.model.get('_audio')._location == "top-left" || this.model.get("_audio")._location == "bottom-right") {
        $(this.el).html(template(data)).appendTo(`.menu-item-${this.model.get("_id")}`);

      } else {
        $(this.el).html(template(data)).prependTo(`.menu-item-${this.model.get("_id")}`);
      }
  ...
  return AudioMenuItemsView;

});


// document.querySelector(".menu-item-inner").innerHTML = "This is Text"