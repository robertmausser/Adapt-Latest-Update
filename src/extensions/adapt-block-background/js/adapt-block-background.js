		...
		onPageReady: function () {
			...
			for (var i = 0, l = this._blockModels.length; i < l; i++) {
				...
				if (!$blockElement.length) {
					self.listenTo(Adapt, 'blockView:postRender', function (view) {
						if (this.$blockElements.hasOwnProperty(view.model.get('_id'))) {
							this.setBlockBackground(view.$el, view.model, view.model.get('_id'));
							this._activeId = view.model.get('_id');
						}
					}.bind(self))
				}
				this.setBlockBackground($blockElement, blockModel, id);
			}
			this._activeId = this._firstId;
		},

		setBlockBackground: function ($blockElement, blockModel, id) {
