define([
  ...
  const HintView = Backbone.View.extend({
    ...
    events: {
      ...
      'click .close-popup': 'onHintToggleClicked',
    ...
    onHintToggleClicked: function (e) {
      ...
      $button
        .toggleClass('is-open', isBeingOpened)
        .toggleClass('is-closed', !isBeingOpened)
        .attr({ "aria-expanded": (isBeingOpened ? true : false) });
      _.delay(function () {
        ...
        if (isBeingOpened) {
          ...
          $widget.removeAttr("tabindex").attr({ "aria-hidden": true });
          if (!e) return; // If the user click event is not occured, the focus should not change.
          $button.focus();
        }
